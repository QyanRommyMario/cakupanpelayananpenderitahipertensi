 ...
 * Menampilkan data Puskesmas berdasarkan role user
 * Admin: Melihat SEMUA puskesmas
 * Puskesmas: Hanya melihat data puskesmas sendiri
 */
export default function DashboardPage() {
  ...
  // Initialize - Fetch user session first
  useEffect(() => {
  ...
  // Fetch global data when period changes or user is loaded
  // KEAMANAN: Filter data berdasarkan role user
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        // KEAMANAN: Pass puskesmasCode untuk non-admin
        const pkmFilter =
          !isAdmin && userPuskesmasCode ? userPuskesmasCode : null;
    ...
    fetchData();
  }, [selectedPeriod, isAdmin, userPuskesmasCode, userLoaded]);

  // Color scale function
  const getScoreColor = (score) => {
    if (score >= 100)
      return { bg: "bg-emerald-100", text: "text-emerald-800", hex: "#10b981" };
    if (score >= 80)
      return { bg: "bg-emerald-50", text: "text-emerald-700", hex: "#34d399" };
    if (score >= 50)
      return { bg: "bg-amber-100", text: "text-amber-800", hex: "#f59e0b" };
    if (score > 0)
      ...
      <div className="p-6 bg-gray-50 min-h-screen">
        <div className="max-w-full mx-auto space-y-6">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
              ...
              {!isAdmin && userPuskesmasCode && (
                <p className="text-sm text-blue-600 mt-1 bg-blue-50 inline-block px-3 py-1 rounded-full">
                  📍 Data Puskesmas:{" "}
                  <span className="font-bold">{userPuskesmasCode}</span>
                </p>
              )}
              {isAdmin && (
                <p className="text-sm text-emerald-600 mt-1 bg-emerald-50 inline-block px-3 py-1 rounded-full">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    {(
                      globalData?.programTotals?.usiaProduktif?.realization || 0
                    ).toLocaleString("id-ID")}{" "}
                    /{" "}
                    {(
                      globalData?.programTotals?.usiaProduktif?.target || 0
                    ).toLocaleString("id-ID")}{" "}
                    sasaran
                  </p>
                </div>
                <div
                  className={`w-14 h-14 rounded-xl flex items-center justify-center ${PROGRAMS.USIA_PRODUKTIF.theme.bgLight}`}
                >
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    {(
                      globalData?.programTotals?.hipertensi?.realization || 0
                    ).toLocaleString("id-ID")}{" "}
                    /{" "}
                    {(
                      globalData?.programTotals?.hipertensi?.target || 0
                    ).toLocaleString("id-ID")}{" "}
                    sasaran
                  </p>
                </div>
                <div
                  className={`w-14 h-14 rounded-xl flex items-center justify-center ${PROGRAMS.HIPERTENSI.theme.bgLight}`}
                >
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    {(
                      globalData?.programTotals?.diabetes?.realization || 0
                    ).toLocaleString("id-ID")}{" "}
                    /{" "}
                    {(
                      globalData?.programTotals?.diabetes?.target || 0
                    ).toLocaleString("id-ID")}{" "}
                    sasaran
                  </p>
                </div>
                <div
                  className={`w-14 h-14 rounded-xl flex items-center justify-center ${PROGRAMS.DIABETES.theme.bgLight}`}
                >
                  <span className="text-2xl">🩸</span>
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    {(
                      globalData?.programTotals?.odgj?.realization || 0
                    ).toLocaleString("id-ID")}{" "}
                    /{" "}
                    {(
                      globalData?.programTotals?.odgj?.target || 0
                    ).toLocaleString("id-ID")}{" "}
                    sasaran
                  </p>
                </div>
                <div
                  className={`w-14 h-14 rounded-xl flex items-center justify-center ${PROGRAMS.ODGJ.theme.bgLight}`}
                >
                  <span className="text-2xl">🧠</span>
                </div>
              </div>
                      ...
                      >
                        {barChartData.map((entry, index) => (
                          <Cell
                            key={`cell-up-${index}`}
                            fill={
                              entry["Usia Produktif"] >= 80
                                ? "#0284c7"
                                : entry["Usia Produktif"] >= 50
                                  ? "#f59e0b"
                                  : "#ef4444"
                            }
                          />
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  ...
                  <p className="text-slate-300 text-sm mt-1">
                    Klik header kolom untuk mengurutkan • Total{" "}
                      ...
                      >
                        {searchQuery ? (
                          <>
                            ...
                            Tidak ada Puskesmas dengan nama &ldquo;{searchQuery}
                            &rdquo;
                          </>
                        ) : (
                          <>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-center">
                            <span
                              className={`inline-block px-3 py-1 rounded-lg font-bold text-sm ${upColor.bg} ${upColor.text}`}
                            >
                              {pkm.usiaProduktif}%
                            </span>
                          </td>
                          <td className="px-4 py-3 text-center">
                            <span
                              className={`inline-block px-3 py-1 rounded-lg font-bold text-sm ${hColor.bg} ${hColor.text}`}
                            >
                              {pkm.hipertensi}%
                            </span>
                          </td>
                          <td className="px-4 py-3 text-center">
                            <span
                              className={`inline-block px-3 py-1 rounded-lg font-bold text-sm ${dColor.bg} ${dColor.text}`}
                            >
                              {pkm.diabetes}%
                            </span>
                          </td>
                          <td className="px-4 py-3 text-center">
                            <span
                              className={`inline-block px-3 py-1 rounded-lg font-bold text-sm ${oColor.bg} ${oColor.text}`}
                            >
                              {pkm.odgj}%
                            </span>
                          </td>
                          <td className="px-4 py-3 text-center">
                            <span
                              className={`inline-block px-4 py-1 rounded-lg font-bold ${avgColor.bg} ${avgColor.text}`}
                            >
                              {pkm.avg}%
                            </span>
                          </td>
                  ...
                  <div className="flex items-center gap-6">
                    <span className="text-gray-600">
                      <strong className="text-emerald-600">
                        {
                          filteredAndSortedData.filter((p) => p.avg >= 100)
                            .length
                        }
                      </strong>{" "}
                      Tuntas
                    ...
                    <span className="text-gray-600">
                      <strong className="text-gray-500">
                        {
                          filteredAndSortedData.filter((p) => p.avg === 0)
                            .length
                        }
                      </strong>{" "}
                      Tidak ada data
            ...
            >
              ...
              <div>
                <p className="font-semibold text-sky-800">
                  Detail Usia Produktif
                </p>
                <p className="text-sm text-sky-600">
                  Lihat rincian per indikator
                </p>
              </div>
            </a>
            <a
          ...
          <div className="text-center text-xs text-gray-400 py-4">
            SMART PTM - Sistem Monitoring Aktual dan Real Time Penyakit Tidak
            Menular • Dinas Kesehatan Daerah Kab. Morowali Utara
          </div>
        </div>
      </div>
    </DashboardLayout>
